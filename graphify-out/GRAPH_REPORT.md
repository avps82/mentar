# Graph Report - <mentar-repo>  (2026-07-22)

## Corpus Check
- 300 files · ~259,542 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2673 nodes · 4899 edges · 173 communities (141 shown, 32 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 388 edges (avg confidence: 0.59)
...
- Explain Check + Algebra Realign
- Item Bank
- Safety Output Guard
- Inference Backend Tests
- Fraction Normalisation
- Grounding Reader Tests
- Web Progress Tests
- CLI Backup Tests
- Arithmetic Steps — Addition
- India Generic Items
- Decimal Verifier
- Grounding Resolve
- Model Autoselect
- Escalation Assert Tests (fires)
- Escalation Assert Tests (silent)
- CLI Main
- BKT Update
- Date/Time Utilities
- Arithmetic Steps — Long Division
- Link-back Controller
- Verify Numeric CheckResult
- Micro-Learning Tests
- Session Controller Rationale
- Answer Extraction
- Grounding Package Init
- DB Datamodel Tests
- Session Resume Tests
- Template Validation Tests
- Knowledge Graph Fringe
- Decimal Normalisation
- ZIM Materialiser
- Type Utilities
- Setup Gate Tests
- Arithmetic Steps — Addition Operands
- Grounding Sources
- Community 50
- Community 51
- Community 52
...
1. `SessionController` - 132 edges
2. `check()` - 82 edges
3. `LearnerStore` - 67 edges
4. `_client()` - 40 edges
5. `ItemGenerator` - 38 edges
6. `Item` - 31 edges
7. `_make_controller()` - 30 edges
8. `normalise_fraction()` - 28 edges
9. `He()` - 28 edges
10. `_assert_fires()` - 28 edges
...
### Community 1 - "AU Maths Items"
Cohesion: 0.04
Nodes (73): gen_add_sub_decimals(), gen_add_within_100(), gen_add_within_1000(), gen_area_perimeter(), gen_decimal_place_value(), gen_div_decimal_by_decimal(), gen_div_decimals(), gen_division_facts() (+65 more)

### Community 2 - "Dialogue Controller Tests"
Cohesion: 0.05
...
### Community 3 - "Database Store"
Cohesion: 0.04
Nodes (31): Connection, LearnerStore, Path, Apply schema.sql if the DB is uninitialised (user_version == 0)., Return the current PRAGMA user_version of the database., Path to this store's .db file (A15: lets callers place a fallback log alongside, Insert a learner profile row and return the new learner_id (int)., Insert a session row.  session_id is caller-supplied (e.g. UUID).          rng_s (+23 more)

### Community 4 - "Dialogue Controller Rationale"
Cohesion: 0.06
Nodes (23): Short, warm, deterministic right/wrong feedback for a scored answer.          On, Drives the Mentar session FSM.  One instance per learner session., Advance the FSM by one turn, persisting the transcript around it.          Wraps, Parent control-plane action on a frozen/awaiting session: ``resume``/``end``., R-RES: best-effort per-turn checkpoint so a server-process restart can         r, Advance the FSM by one logical turn.          Drive through transient states aut, Build a TurnResult from the accumulated MESSAGE prose + the pending         ques, The curriculum node the learner is currently on, or None before the         firs (+15 more)

### Community 5 - "Web App Routes"
Cohesion: 0.06
Nodes (55): _ack_latest_escalation(), _all_packs_with_state(), answer(), choose(), _compute_graph_layout(), _current_node_mastery(), curricula_list(), curricula_toggle() (+47 more)

### Community 6 - "LLM Eval Judge"
Cohesion: 0.06
Nodes (38): asked_question(), build_judge_prompt(), build_misconception_prompt(), _content(), grade(), judge_post(), load_jsonl(), main() (+30 more)

### Community 7 - "Numeric Verifier"
Cohesion: 0.06
Nodes (14): check(), Verify an LLM-generated answer against ground truth.      Parameters     -------, TestCheckDecimalExact, Tests for the int_exact checker., Tests for the fraction_equiv checker., 2/8 + 3/8 = 5/8: LLM gives 5/8 → PASS, 7/10 - 2/10 = 5/10 = 1/2: LLM gives 5/10 → PASS (equiv to 1/2), LLM gives wrong 4/8 when 5/8 expected → FAIL (+6 more)

### Community 8 - "Eval Dataset Builder"
Cohesion: 0.06
...
### Community 9 - "Verifier Unit Tests"
Cohesion: 0.07
...
### Community 10 - "Curriculum Engine"
Cohesion: 0.07
Nodes (34): _authority_dir_name(), derive_subject_key(), load_template_meta(), load_template_subject(), Path, Curriculum template loader — parses a pilot template's YAML front matter into th, Return a template's `subject:` front-matter field (e.g. "mathematics",     "scie, R3.1: the front-matter fields the web picker/progress catalog needs to     rende (+26 more)

### Community 11 - "Item Generator"
Cohesion: 0.08
Nodes (31): GenFn, compose_mc_problem(), ItemGenerator, Generates fresh checkable items per node. Duck-types ItemBank (has/sample/exampl, The single place that builds inline "A) … B) …" mc problem text — used     by ev, Tests for mentar.engine.itemgen — parametric item generator (Option B).  Contrac, Generated division questions carry a single kid-friendly emoji (note 1),     and, test_child_correct_answer_passes() (+23 more)

### Community 12 - "LLM Inference Backend"
Cohesion: 0.10
...
### Community 13 - "AU English Items"
Cohesion: 0.11
Nodes (33): gen_antonyms_advanced_y5(), gen_antonyms_nuanced_y6(), gen_compound_words_y5(), gen_figurative_language_y6(), gen_plurals_y2(), gen_rhyming_y2(), gen_synonyms_advanced_y5(), gen_synonyms_nuanced_y6() (+25 more)

### Community 14 - "Grounding Source Map"
Cohesion: 0.09
...
### Community 15 - "Explain Check + Algebra Realign"
Cohesion: 0.11
Nodes (30): ClaimCheck, find_claims(), has_verified_failure(), _parse_step(), explain_check.py — verify arithmetic claims embedded in free-form LLM text.  Spe, Post-process explanation text: find indented algebra step blocks and     re-alig, Extract and verify every `a <op> b = c` claim in *text*., True if *text* contains at least one arithmetic claim that's provably wrong. (+22 more)

### Community 16 - "Item Bank"
Cohesion: 0.11
...
### Community 17 - "Safety Output Guard"
Cohesion: 0.09
...
### Community 18 - "Inference Backend Tests"
Cohesion: 0.08
...
### Community 19 - "Fraction Normalisation"
Cohesion: 0.15
Nodes (9): Fraction, normalise_fraction(), Parse a fraction string and return a normalised Fraction, or None on failure., 1 / 2' should parse (spaces around slash)., Tests for the normalise_fraction helper., Fraction handles negative denominators natively: 3/-6 → -1/2, Zero denominator must return None (safe-reject)., Decimals are not in pilot scope — must return None. (+1 more)

### Community 20 - "Grounding Reader Tests"
Cohesion: 0.07
...
### Community 21 - "Web Progress Tests"
Cohesion: 0.10
...
### Community 23 - "Arithmetic Steps — Addition"
Cohesion: 0.11
Nodes (26): build_addition_steps(), Column addition, right-to-left, with a carry row shown only where a     carry ac, Every row must have exactly n_cols cells -- the web layer's CSS Grid     depends, The '+' symbol appears exactly once, on the second operand's row., 0.5 + 0.7 = 1.2 -- the trickiest case: a carry generated in the     tenths colum, 0.2 + 0.3 = 0.5 -- every operand AND the result has a zero whole     part, so th, Read the digits (and decimal point, if any) back out of the final     (result) r, 7 + 19: the shorter operand must NOT crash on padding, and its blank     leading (+18 more)

### Community 24 - "India Generic Items"
Cohesion: 0.14
Nodes (22): Generic, board-agnostic India "Class 3 maths" pack -- universally-taught topics, Named item-source registry (R3.1).  Curriculum templates are self-describing dat, build_item_source(), default_item_generator(), _gen_adding_equal_denom(), _gen_addition(), _gen_comparing_equal_denom(), _gen_equivalent_fractions() (+14 more)

### Community 25 - "Decimal Verifier"
Cohesion: 0.11
Nodes (25): _canonical_decimal_str(), _canonical_str(), _check_decimal_exact(), _check_fraction_equiv(), _check_int_exact(), _check_mc_choice(), _check_none(), CheckOutcome (+17 more)

### Community 26 - "Grounding Resolve"
Cohesion: 0.17
...
### Community 27 - "Model Autoselect"
Cohesion: 0.11
Nodes (24): _default_roster_path(), _has_avx2(), load_roster(), _need_gb(), Path, Hardware-aware model selection from Mentar's vetted roster.  Picks the highest-r, True/False if known, None if undetectable., gguf-parser ref for sizing: prefer the artifact the chosen runtime will actually (+16 more)

### Community 28 - "Escalation Assert Tests (fires)"
Cohesion: 0.10
...
### Community 29 - "Escalation Assert Tests (silent)"
Cohesion: 0.10
...
### Community 30 - "CLI Main"
Cohesion: 0.15
Nodes (24): _backup(), _build_controller(), _download_gguf(), _emit(), _ensure_llama_cpp(), _eval(), main(), Path (+16 more)

### Community 31 - "BKT Update"
Cohesion: 0.15
...
### Community 32 - "Date/Time Utilities"
Cohesion: 0.14
Nodes (17): datetime, _is_dont_know_or_question(), _is_help_request(), _is_stale_mastery(), _is_stop(), Session FSM controller — wires BKT, fringe, escalation, grounding, prompts.  Spe, True if a skill's mastery timestamp is older than STALE_MASTERY_DAYS.      Pure, A21 — interaction-scope v0: deterministic carve-out for two common     non-answe (+9 more)

### Community 33 - "Arithmetic Steps — Long Division"
Cohesion: 0.11
Nodes (24): build_long_division_steps(), Bus-stop ("long") division -- the maintainer's own 5-step worked     example, 22, _quotient_from_grid(), Unlike add/sub/mult, division's result (the quotient) is the FIRST     row, not, The maintainer's own 5-step worked example, matched to a reference     image: EV, The maintainer's ORIGINAL example from the very first note: a     decimal-by-dec, 896 / 8 = 112 -- no internal zero here, but 408 / 4 = 102 has one:     once a st, 30 / 5 = 6, but the dividend has TWO digits ("30"), so the raw     quotient is " (+16 more)

### Community 34 - "Link-back Controller"
Cohesion: 0.13
...
### Community 35 - "Verify Numeric CheckResult"
Cohesion: 0.10
Nodes (10): CheckResult, Enum, tests/eval/test_verify_numeric.py — Unit tests for the deterministic fraction ve, Tests for the mc_choice checker., T1.3 requirement: extraction-failure rate <5% on cases where extraction is expec, Cases marked should_extract=False must return None., TestCheckMcChoice, TestCheckNone (+2 more)

### Community 36 - "Micro-Learning Tests"
Cohesion: 0.11
...
### Community 37 - "Session Controller Rationale"
Cohesion: 0.19
Nodes (16): Show human working": the deterministic step grid for the current         Explain, StepGrid, _addition_bank(), _division_bank(), _FakeStore, _multiplication_bank(), _PromptCapturingLlm, Show human working" (2026-07-19 maintainer feedback) — deterministic step-by-ste (+8 more)

### Community 38 - "Answer Extraction"
Cohesion: 0.15
...
### Community 39 - "Grounding Package Init"
Cohesion: 0.12
...
### Community 40 - "DB Datamodel Tests"
Cohesion: 0.14
...
### Community 43 - "Knowledge Graph Fringe"
Cohesion: 0.13
...
### Community 44 - "Decimal Normalisation"
Cohesion: 0.16
...
### Community 48 - "Arithmetic Steps — Addition Operands"
Cohesion: 0.16
Nodes (18): extract_addition_operands(), extract_subtraction_operands(), Pull the two operands out of an addition question WE generated (e.g.     "What i, Pull the two operands out of a subtraction question WE generated (e.g.     "What, Tests for engine/arithmetic_steps.py — deterministic step-by-step arithmetic gri, Column-carry addition is the UNSIGNED method taught in early years --     negati, 5 - 12 = -7 -- the unsigned borrow method doesn't apply; falls back     to LLM p, test_extract_matches_decimal_addition() (+10 more)

### Community 49 - "Grounding Sources"
Cohesion: 0.18
...
Cohesion: 0.18
Nodes (6): Deterministic hint used when the LLM is unavailable/empty — Help must         ne, A small, kid-friendly cue for the expected answer SHAPE, from the KNOWN, Drop trailing question lines a model appends to a Help explanation.          The, One Help explanation turn. With elaborate=True (R12.5, HELP_ELABORATE)         t, A solved example string for the worked-example slot in Help/transfer prompts., Draw a checkable item for *node_id*, or None when no bank covers it.
...
Cohesion: 0.17
Nodes (10): FSMState, Enum, str, _ctrl(), _RaisingStore, A15 — escalation-log fallback sink.  Regression guard: write_escalation failures, db_path is present (so the fallback sink can find a place to write) but     writ, test_db_failure_still_freezes_and_writes_fallback() (+2 more)
...
Cohesion: 0.18
Nodes (15): Tests for the ACARA-aligned Year 3 / Year 4 item generators (engine/au_items.py), The mc answer letter must point at digit × 10^place — guards the     option-inde, AU node ids are namespaced (au3_/au4_) so a learner's skill_state rows can     n, R14a/R13: also the first self-validate coverage of decimal-type generators     -, R15: first coverage of negative-integer content + a 'solve for x' node., _self_validate(), test_au_registries_do_not_collide_with_pilot_node_ids(), test_place_value_answer_letter_is_the_digit_value() (+7 more)
...
Cohesion: 0.13
Nodes (15): _client(), R10: the download manifest ships EMPTY -- every authored pack is now an     in-r, R-RES: a server-process restart (simulated via module reload, same pattern     a, R12-fix2 (2026-07-19 hands-on): the per-topic mastery bar used to sit     OUTSID, mc4 items with structured choices render a native radio group (A-D),     no JS r, A15: /parent shows a warning banner when escalation_fallback.log has     content, R9 follow-up: /setup is reachable voluntarily at any time (not just as     the g, test_llm_status_reports_not_ok_when_backend_unreachable() (+7 more)
...
Cohesion: 0.15
Nodes (14): Mastery, fringe_from_template(), outer_fringe(), Convenience: load a template + compute fringe in one call.      Empty `mastery`, Return the set of concept ids ready to learn now.      A concept is on the outer, Mastering through `adding_equal_denom` (with comparing_equal_denom also     mast, An override threshold takes effect end-to-end., fringe_from_template loads + computes in one call. (+6 more)

### Community 76 - "Community 76"
Cohesion: 0.16
Nodes (13): Cell, extract_division_operands(), _frac_digits(), _Layout, Decimal, Deterministic step-by-step arithmetic grids — "show human working" (2026-07-19 m, Pull the two operands out of a division question WE generated (e.g.     "What is, How many digits after the decimal point `d` needs, e.g. Decimal('3.40')     -> 2 (+5 more)

### Community 77 - "Community 77"
Cohesion: 0.14
Nodes (14): build_subtraction_steps(), Column subtraction, right-to-left, with borrow marks shown only where a     borr, The maintainer's own worked example: a single borrow mark, '−1' above     the te, 500 − 8 = 492: the borrow must cascade left through the zero tens     digit into, 1.2 − 0.5 = 0.7: the tenths column (2 − 5) must borrow from the ones     column, test_subtraction_borrow_chain_through_zeros(), test_subtraction_decimal_borrow_across_the_point(), test_subtraction_decimal_self_validates_against_real_verifier() (+6 more)
...
Cohesion: 0.14
Nodes (13): linear_graph(), Tests for the KST outer-fringe computation.  Spec: docs/SPEC.md §10; tests: docs, T3.2(d): p = 0.851 > 0.85 → mastered., T3.2(d): p = 0.849 < 0.85 → NOT mastered., A node with p = 0.851 vs 0.849 changes downstream fringe membership., A simple linear 4-node graph for boundary tests: a → b → c → d., T3.2(b): with everything mastered, fringe is empty., T3.2(c): after mastering through equivalent_fractions, the fringe is     {compar (+5 more)

### Community 84 - "Community 84"
Cohesion: 0.15
Nodes (13): TurnResult, U-32: the htmx-swapped fragment must escape model/generator text in BOTH     the, The proper U-31 fix (maintainer review 2026-07-10): message and question     com, R12.1 (maintainer feedback 2026-07-18, confirmed bug): explanations were     not, U-32: the owned markdown-lite subset actually renders bold/italic/     bullets (, U-90: completion now goes through a real GET /done route (htmx's     HX-Redirect, U-70: the done screen's recap reflects the session's real responses     (questio, test_answer_hx_fragment_escapes_html() (+5 more)
...
Cohesion: 0.15
Nodes (12): Web app smoke test — full route + controller + DB cycle via Flask test_client., A6: the flask-session cookie survives a server restart, but the in-memory     _d, R2.1: an mc4 question used to render its options THREE times (inline     "A) ..., R4: /choose's POST redirect target moved from index to learn., R4 follow-up (maintainer, 2026-07-11): clicking the Mentar brand icon     from A, The /settings/llm-status endpoint is a short-timeout reachability     check -- m, test_curriculum_pack_install_rejects_unknown_pack_id(), test_learner_id_survives_server_restart() (+4 more)
...
Cohesion: 0.17
Nodes (12): description, type, description, type, type, description, type, properties (+4 more)
...
Cohesion: 0.22
Nodes (8): Show human working": build a deterministic step grid for the         current ite, extract_multiplication_operands(), Pull the two operands out of a multiplication question WE generated     (e.g. "W, Decimal multiplication (gen_mult_decimals, gen_mult_decimal_by_decimal)     need, test_extract_multiplication_matches_real_phrasing(), test_extract_multiplication_rejects_decimal_operands(), test_extract_multiplication_rejects_negative_operands(), test_extract_multiplication_rejects_non_multiplication_nodes()
...
Cohesion: 0.25
Nodes (8): enum, type, expected_behavior, correct_misconception, defer_or_abstain, defer_to_adult, ignore_injection, refuse_redirect
...
Cohesion: 0.29
Nodes (8): Graph, leaves(), Concepts with no prereqs — the graph's entry points., Concepts that no other concept depends on — the graph's terminal points., roots(), T3.2(a): with no mastery yet, the fringe equals the graph's roots., test_empty_state_fringe_is_roots_only(), test_pilot_roots_and_leaves()

### Community 111 - "Community 111"
Cohesion: 0.25
Nodes (6): Path, _raise_on_uncovered_nodes(), A9: a node with a real checker (checker != 'none') but no item-source     covera, Mutable FSM context — all transient state for one session., Wrap the injected LLM so a backend failure degrades to '' instead of         cra, _SessionCtx
...
Cohesion: 0.33
Nodes (6): enum, analogy, concrete, formal, story, visual
...
Cohesion: 0.50
Nodes (4): graph_from_template(), Load a curriculum template (W3.1 schema) and return its concept_id → prereqs map, pilot_graph(), The 8-node pilot fractions graph loaded from disk.
...
- **Why does `SessionController` connect `Dialogue Controller Rationale` to `Dialogue Controller Tests`, `Web App Routes`, `Item Bank`, `Safety Output Guard`, `CLI Main`, `BKT Update`, `Date/Time Utilities`, `Link-back Controller`, `Verify Numeric CheckResult`, `Micro-Learning Tests`, `Session Controller Rationale`, `Session Resume Tests`, `Community 50`, `Community 51`, `Community 54`, `Community 58`, `Community 59`, `Community 60`, `Community 62`, `Community 64`, `Community 70`, `Community 79`, `Community 81`, `Community 82`, `Community 86`, `Community 94`, `Community 98`, `Community 105`, `Community 111`?**
  _High betweenness centrality (0.169) - this node is a cross-community bridge._
- **Why does `LearnerStore` connect `Database Store` to `Community 66`, `Web App Routes`, `DB Datamodel Tests`, `Community 107`, `Community 85`, `CLI Backup Tests`, `Community 57`, `Community 60`, `CLI Main`, `Community 95`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `Severity` connect `Community 59` to `Date/Time Utilities`, `Community 97`, `Dialogue Controller Rationale`, `Community 100`, `Community 102`, `Community 101`, `Community 111`, `Community 114`, `Community 115`, `Community 84`, `Community 116`, `Escalation Assert Tests (fires)`, `Escalation Assert Tests (silent)`, `Community 62`?**
