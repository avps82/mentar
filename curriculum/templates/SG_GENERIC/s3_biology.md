...
concepts:
  ...
  - id: sg_s3_cell_transport
    ...
    grounding: {}
    transfer_seeds:
      - "Which of these is DIFFUSION of a gas or solute (not water — water is osmosis)? A) oxygen moving from the alveoli into the blood  B) a raisin swelling in pure water  C) water entering a root hair cell  D) the gut absorbing glucose when blood glucose is already higher. Answer with the letter."
    verifier:
      answer_type: mc4
      checker: mc_choice
  ...
  - id: sg_s3_photosynthesis_respiration
    ...
    grounding: {}
    transfer_seeds:
      - "Which of these is a PRODUCT of photosynthesis? A) carbon dioxide taken in by the leaf  B) light energy from the Sun  C) water drawn up from the roots  D) glucose stored as the plant's food. Answer with the letter."
    verifier:
      answer_type: mc4
      checker: mc_choice
    bkt_priors: { guess: 0.25, slip: 0.1, learns: 0.2, forgets: 0 }
