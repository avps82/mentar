...
| **2 — correctness, Gemma-drafted/Opus-verified** | A17 → A16 → A5 → A6 → A9 → A7 → A20 → A21 | 🔶 A17 (#60) + A16 (#61) + A5 (#62) + A6 (#63) + A9 done (PR pending), rest not started | A17 before A16 (layering clean-up first makes A16's startup hook land in one place). A20/A21 are the new ratified tasks — specs below. |
