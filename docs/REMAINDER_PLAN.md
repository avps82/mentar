...
| **3 — assent/docs/hygiene** | A1 → A4 → A18 → A19 → A2 → A11 | 🔶 A1 found **already shipped** (commit `e664658`, doc drift corrected) + A4 + A18 + A19 done (PR pending); A2/A11 not started | A1 before A4 (same controller preamble). A11 (FSM conformance test) runs **after** waves 1–2 since A9/A14/A21 add new FSM transitions the doc needs to capture. A18's own re-run caught + fixed a real eval-harness bug (see docs/MODEL.md 2026-07-05 note) — the A7 prompt change itself was confirmed safe (20/20, no regression). A19 bumped schema to v3 (`session.rng_seed`). |
