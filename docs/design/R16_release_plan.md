...
| D3 | W2.2 professional safeguarding review | **Reframed 2026-08-12 — sought pro-bono, not a funded gate** | Handoff wording + emergency-services signposting. As an unfunded OSS project Mentar cannot commission this; it is actively sought pro-bono (`SAFEGUARDING_REVIEW_PACKET.md` is ready to hand to any volunteer) and gates **unsupervised** mode specifically — NOT the supervised pilot, whose safety rests on the present-adult constraint. Absent a volunteer, the maintainer may substitute an explicit documented risk decision. Full honest position: SAFETY §3.5.1. |
| D4 | Secret + history sweep | ✅ **DONE 2026-08-11 (maintainer directed):** `gitleaks git . --no-banner --redact` (v8.30.1, the exact CI command) — 309 commits scanned, **no leaks found**. Supplementary checks: this session's own local-LLM gateway token confirmed NEVER in history (`.claude/settings.local.json` is gitignored and has zero commits touching it, ever); a secret-pattern regex sweep beyond gitleaks' ruleset (api_key/secret/password/bearer assignments, private-key headers) — the one PRIVATE KEY hit is the literal regex pattern INSIDE the pre-commit hook's own secret-scanner script, not a real key; all 4 unique 64-char hex strings in history individually verified as legitimate content hashes (an htmx subresource-integrity hash, a dataset checksum, a curriculum-pack manifest hash), not credentials. **`graphify-out/` — RESOLVED 2026-08-11 (maintainer chose the split).** `graph.json`, `graph.html` and `manifest.json` are now gitignored: the first two were ~3.5 MB each and rewritten wholesale on every refresh, so each regeneration added that much to history permanently, and all three are reproducible from a graphify run. `manifest.json` is dropped WITH them deliberately — it is `graph.json`'s incremental-update bookkeeping, and shipping it alone would leave a fresh clone with a manifest claiming everything is extracted and no graph to merge into. Still committed: `GRAPH_REPORT.md` (human-readable), the README-embedded PNG, and `cost.json`. Never a secret risk either way — this was repo-weight hygiene, as originally noted. |
...
| D8 | Doc truth pass | ✅ **Done 2026-08-23** | The overnight sweep of 2026-07-13 found **4 stale `🔭` rows in `PHASE0_STATUS.md` that had actually shipped**. Never trust a `🔭` marker without verifying against the tree. **2026-08-23 audit:** every doc mechanically checked for symbols, test names and entity counts that no longer match the code. Found and fixed: this table's own E2 (shipped, marked open) and E1 (draw-dependent now 0); `_split_turn_text()` in UI_REQUIREMENTS (superseded mechanism documented as current); a `_frac_digits` typo. Everything else flagged was checked in context and correct — `TESTS.md`'s planned-vs-actual mapping table, `SESSION_FSM.md` correcting its own earlier false claim, and scoped counts read as global ones. |
...
|---|---|---|---|
| E1 | **2 of 5 explain-method findings fixed, 3 remain** — the 2 scaffold-routing bugs ✅ fixed 2026-08-10 (Findings 1+2); the decimal-mult-almost-never-eligible + 2 draw-dependent step-grid bugs (Findings 3+5) have a written plan (`docs/design/step_grid_signed_and_decimal_mult_design.md`, 2026-08-11) but Phase C is gated on maintainer worked examples, not built. **Partly stale, corrected 2026-08-23 doc audit:** `python3 -m mentar.tools.audit_explain_paths` now reports **DRAW-DEPENDENT: 0** across all 934 nodes (and 0 prose-only, 0 unscaffolded), so the draw-dependent half closed with the Phase A+B step-grid work of 2026-08-12. Only the Phase C shapes gated on worked examples remain | `docs/EXPLAIN_METHOD_AUDIT.md` (2026-07-25, audit-only) | Real, plan exists for the rest. The decimal-mult one was caught **only by running real draws** — fix the same way, don't reason from the code |
| E2 | Restart button | `project_r12_followup_bugs` | ✅ **SHIPPED — row was stale, corrected 2026-08-23 doc audit.** `POST /restart` (`web/app.py`), the "Start again" button in `done.html`, and five tests in `tests/web/test_restart_after_stop.py` (incl. one asserting restart does NOT release an escalation freeze). Built 2026-08-15 after the maintainer reported "when stopped... I can see progress but can't start again". The design question this row was waiting on — in-process restart for a WSGI server — was sidestepped: it clears the cached ended controller rather than restarting the process |
...
| H7 | B0 — verifier ceiling decision | ✅ **SUPERSEDED next morning — BUILT.** Overnight decision was cap-at-Y8 (sympy uninstallable by agent); maintainer ran the install 2026-08-11 morning, so the `expression` answer type shipped same day: `expression_equiv` checker in verify_numeric.py, additive-only (R13 shape), lazy sympy import (module works without it — SAFE_REJECT with clear detail), strict pre-parse gate (char allowlist + single-letter-names + 100-char cap — the gate, not sympy, is the safety boundary), `simplify(candidate - truth) == 0` equivalence with implicit multiplication + `^` for kid input. 31 new tests incl. an extend-never-weaken hostile-input class. sympy>=1.12 added to pyproject dependencies. **Y9-12 maths authoring (B1) is now unblocked** | **Decided tonight: cap at Y8.** Not a change of recommendation — sympy is simply not installed and the sandbox denies agent-chosen `pip install` (`feedback_sandbox_blocks_agent_binary_exec`); adding the dependency is a maintainer action (add to `pyproject.toml` + install). The `expression` answer-type design stays as specced in B0; build resumes when sympy is present. Y9-12 maths authoring stays blocked behind it |
...
| ~~`chempy`~~ | **DROPPED same day** — maintainer: "keep it minimal... see if the core is enough." It is: chempy's HARD requirements (not extras) include matplotlib + the pyneqsys/pyodesys ODE stack + dot2tex, and the one needed function (equation balancing) is textbook nullspace linear algebra over the element-count matrix — which core sympy does exactly. Replaced by owned thin glue `engine/chem_balance.py` (~110 lines incl. strict formula parser), validated **byte-identical to chempy on a 10-equation education battery before chempy was removed** — the battery is pinned in `tests/engine/test_chem_balance.py` so the owned balancer can never silently drift from the reference. Refusal-over-guessing on impossible/non-unique balances. Ceiling (ponytail-noted in the module): neutral molecules only — ionic charge balancing is one more conservation row when Y11+ redox content is authored, not a new dependency. chempy + 28 transitive packages uninstalled | higher-grades chemistry wave |
...
| **∞** | D3 safeguarding review — sought pro-bono (SAFETY §3.5.1), not funded | Gates **unsupervised** mode, not the supervised pilot; welcomed if a professional volunteers, else a documented maintainer risk decision substitutes |
