...
- **Deviation — one observation per attempt (2026-09-02, W3.3 §3.3):** the Help loop re-checks the SAME item up to three times; each re-check was a fresh BKT observation, so one hard item could produce four correlated wrongs (0.70 → 0.0003 with the shipped priors). Now the first scored attempt on an item is always observed (hinted or not), a hinted correct is always observed, and a later hinted wrong on that item is logged but not observed. Max two observations per item; `recompute_mastery` replays the same rule.
- **Display:** `p_mastery` is never shown as a percentage — its parameters are hand-set, never fitted (W3.3 §6 G1) — only as one of five bands whose top starts at the 0.85 graduation threshold (`web/app.py::mastery_band`).
