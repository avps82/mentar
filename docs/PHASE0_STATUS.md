...
**Tests:** `tests/grounding/` — 7 files, **50 tests, all pass** (`pytest tests/grounding/`, libzim available in sandbox; SMB mocked via `sys.modules`, no live server needed). Each file carries an inline `python3` smoke runner + pytest fns (project convention). **Remaining (Sonnet wiring):** dialogue controller calls `resolve_grounding` into the `{{grounding_passage}}` slot.
