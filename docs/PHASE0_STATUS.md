...
| W7.4 ZIM acquisition | 🟡 | Tooling ✅: `scripts/fetch_pilot_zims.sh` (Kiwix) + `.gitignore *.zim` + programmatic fixture ZIM (`tests/fixtures/build_fixture_zim.py`, binary gitignored, built at test time). **Pending (manual/Pradeep):** download the real Vikidia EN + Simple English Wikipedia ZIMs + verify the reader's path convention against a real modern ZIM (contract verification step 2). |
...
**Tests:** `tests/grounding/` — 5 files, **38 tests, all pass** (`pytest tests/grounding/`, libzim available in sandbox). Each file carries an inline `python3` smoke runner + pytest fns (project convention). **Remaining (Sonnet wiring):** dialogue controller calls `resolve_grounding` into the `{{grounding_passage}}` slot.
