...
| 2026-06-15 | Grounding reader decision — verified the ZIM-MCP option = **OpenZIM MCP** (`cameronrye/openzim-mcp`, **MIT**, libzim, maintained). **ADOPT it for the ZIM-reading layer** (vs build), used **deterministically** (not model-driven tools) in the pilot; build only the scope/safety wrapper. MIT = compatible with any W4.2 licence. |
