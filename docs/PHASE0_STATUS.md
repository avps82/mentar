...
| 2026-06-15 | **W1.1 endpoint live + W1.2 roster complete** (Pradeep stood up the eval host; Opus recorded). OpenAI-compatible **LiteLLM proxy** at `http://192.168.xx.xxx:4000/v1` now serves the full cross-family candidate set — `llama3.1:8b`, `gemma2:9b`, `phi4-mini`, `qwen3:14b`, `qwen3.5:9b`, `qwen3.5:2b` + `mistral-small3.1` (ceiling, not candidate) + Sonnet (judge). Three verified responding. New `docs/MODEL.md` = canonical roster + roles A–D + size→tier map + eval dimensions (incl. latency) + run plan. Config `vllm:` block points at the proxy (token via env, never committed). **Ready to run** once T1.1 dataset is built. |
