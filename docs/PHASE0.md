...
| W6.2 | **Prompt template set v0 — versioned files.** One template file per: interaction pattern (×3, 12), Help modality (×5, 13.2), transfer-question generation, system prompt (with safety framing + grounding-as-data wrapper per W2.3). Stored under `prompts/` with a version hash; the dialogue controller loads them — never hardcoded strings. T7.3 regression triggers on any change to these files. | W2.3 (injection framing) | `prompts/` dir exists with ≥10 template files + `prompts/README.md` registry (file, purpose, version hash); T4.6 loads-and-versions test passes | 12, 13.2, T7.3, T4.6 |
...
| W6.5 | **(POST-PILOT / Phase-1)** **Interactive manipulatives — Mentar-owned widgets.** Generated, parameterized-per-concept interactive graphics the child can change (e.g. draggable fraction bar, splittable pie), rendered in the pilot web app. **NOT** retrieved from ZIM (avoids unvetted ZIM-JS in a child webview; pedagogically owned is better). Needs a curriculum-schema field to attach a widget type + params per node, and a small owned widget library. Decision/scope only here; build is Phase-1 UX. | W6.3 | design note + curriculum-schema extension + ≥1 fractions manipulative in the web app | 4, 12, W6.3, `docs/design/media_and_interactivity.md` |
...
| W7.4 | **ZIM acquisition + NAS/Samba** — `scripts/fetch_zim.py` pulls Vikidia EN + Simple English Wikipedia from a **Kiwix mirror list** to a **local / mounted-NAS / `smb://`** destination. Read side (`grounding/sources.py`) accepts the same three location forms; `smb://` ZIMs copied once to `zim_cache_dir` (optional `[nas]` extra = `smbprotocol`). `.gitignore *.zim` + programmatic fixture ZIM (`tests/fixtures/`). Binaries NOT committed. **Future:** Kiwix OPDS catalog auto-discovery. | — | script + reader accept local/NAS/SMB; `test_sources.py` passes (SMB mocked); fixture ZIM drives the suite | W4.1, W7-design |
...
| W7.6 | **(POST-PILOT / presentation)** **Vetted ZIM media serving.** Serve static media (images, later audio/video) from a media-rich ZIM to the pilot web app via `libzim` (any entry → bytes + mimetype). Gating concern is **child-safety vetting** — NOT rendering arbitrary ZIM media/HTML/JS to a child; needs a curated/whitelist path + a `grounding:`-schema media field. Distinct from W6.5 (owned interactive widgets). Interactive ZIM-JS (e.g. PhET sims) would also fall under this vetting bar. Decision/scope only here. | W7.1, W6.3 | design note + media-serving path + vetting/whitelist mechanism | 15, SAFETY §1.5, `docs/design/media_and_interactivity.md` |
