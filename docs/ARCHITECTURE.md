...
| `src/mentar/engine/arithmetic_steps.py` | Deterministic step-grid builder for column arithmetic (add/sub/mult/long-division with carry/borrow marks); wired into `HELP_ELABORATE`. Phases A+B (2026-08-12) added decimal multiplication (`build_multiplication_decimal_steps` — points stripped, product's places SUMMED not maxed) and signed multiplication (`build_signed_multiplication_steps` — sign rule stated, then magnitudes). Both wrap the unchanged partial-products builder; each has a sibling extractor so `extract_multiplication_operands` keeps its integer-only contract | R12-show-working; E1 Findings 3+5 |
...
| `src/mentar/tools/` | CLI utilities: `validate_template.py` (DAG/schema validator); `check_doc_paths.py` (fails the suite when a prose doc names a path that doesn't resolve); `audit_explain_paths.py` (reports what "explain" produces per node — step-grid vs prose vs draw-dependent, and scaffold routing — by running the real generators, not reading regexes) | W3.1; T3.1; doc-audit 2026-08-11; E1 audit 2026-08-12 |
| `src/mentar/cli/` | `mentar` CLI entry point (`__main__.py`); subcommands: `setup`, `serve`, `eval`, `validate-template` | PHASE0.md W6.4 |
| `curriculum/templates/` | Country/grade curriculum templates: `_pilot/` (fractions Phase-0), `AU_ACARA/` (maths Y1–12, English Y1–12, science Y1–10, senior years split into biology/chemistry/physics/earth-env, literature and four maths courses), `IN_GENERIC/` (maths/English/science Classes 2–12, senior bio/chem/physics C11–12), `SG_GENERIC/` (Primary 2–6, Secondary 1–4, senior bio/chem/physics S3–4), `US_GENERIC/` (Grades 2–12, bio/chem/physics G9–11), `practice/` (country-agnostic). The three `*_GENERIC` packs claim **no** syllabus alignment — see `docs/CONTENT_LICENSES.md` §2b | SPEC §9; W3.1–W3.2; R14a; R15; B3–B5 |
